...
const ProductionAct = () => {
  ...
  const { platform, isWeb } = useContext(PlatformContext);
  ...
  useEffect(() => {
    ...
    // Проверяем наличие прогноза в localStorage
  ...
  const handleSaveAndPrint = async () => {
    ...
    try {
      ...
      // Если это не веб-версия, отправляем на принтер
      if (!isWeb) {
        const printResult = await sendToPrinter(savedDoc);
        
        if (printResult) {
          toast.success("Документ успешно сохранен и отправлен на печать");
        } else {
          // Сообщение об ошибке уже показывается в sendToPrinter
          toast.success("Документ сохранен, но возникли проблемы с печатью");
        }
      } else {
      ...
      toast.error("Произошла ошибка при сохранении и печати");
    } finally {
      setIsPrinting(false);
    }
  };
